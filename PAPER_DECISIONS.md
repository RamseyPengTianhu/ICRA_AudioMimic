...
## Narrative hierarchy implementation (2026-09-11)

User approved the hierarchy plan with one explicit amendment: retain the teammate's execution material in the main text. The main contribution order is now anticipatory conditioning, kinematics-guided D+C with paired training, and Commit Forcing. The execution-interface subsection and diagram remain in the main paper, with a concise account of the historical replay diagnostics and their scope. The supplementary execution measurements and teammate CoF diagram remain available.

Implementation compresses the introduction and related work, moves representation/optimization/evaluator details to the supplement, and brings the accepted vector overview to page 2. D+C and CoF diagrams remain separate at readable widths. The paper no longer displays empty comparison tables or inherited plans for blind ratings, wrong-time interventions, and runtime benchmarks. Matched forecast, representation and continuation comparisons remain explicitly pending in Discussion. Their proof roles and approved scope remain in section 15 of the canonical paper revision plan; the pre-edit experimental source is archived for provenance, not as authorization to run its superseded protocols.
