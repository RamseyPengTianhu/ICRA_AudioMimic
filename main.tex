...
Facet & Endpoints & Facet & Endpoints \\
\midrule
\textbf{Rhythm} & BAS $\uparrow$; beat precision/recall/F1 $\uparrow$; timing
$\rightarrow0$; offbeat rate $\downarrow$ &
\textbf{Realism} & FID$_k$/FID$_g$ $\downarrow$ against matched ground truth \\
\textbf{Tempo and phase} & BPM/period error $\downarrow$; accent phase error $\downarrow$ &
\textbf{Diversity} & Div$_k$/Div$_g$ and same-song diversity $\rightarrow$ GT \\
\textbf{Dynamics} & Onset--impact correlation $\uparrow$; response lag
$\rightarrow0$; energy-envelope response $\uparrow$ &
\textbf{Smoothness} & Velocity/acceleration and jerk $\rightarrow$ GT; jitter and
boundary discontinuity $\downarrow$ \\
\textbf{Musical structure} & Phrase/section response $\uparrow$; transition timing
$\rightarrow0$; long-horizon fit $\uparrow$ &
\textbf{Activity} & Motion energy, static/freezing ratio, and repetition $\rightarrow$ GT \\
\textbf{Cross-modal semantics} & Audio--motion R@K $\uparrow$; multimodal distance
(MMDist) $\downarrow$ &
\textbf{Physical plausibility} & PFC, FSR, and penetration $\downarrow$; root and
joint-limit stability $\uparrow$ \\
\textbf{Style and emotion} & Frozen genre/mood agreement and blinded fit ratings
$\uparrow$ &
\textbf{Perception} & Blinded naturalness, aesthetics, smoothness, and expressiveness
$\uparrow$ \\
\bottomrule
\end{tabular}
\end{table*}
...
The available rhythm diagnostics (BAS $0.258$, Beat F1 $0.227$, recall $0.187$, and
offbeat rate $0.696$) localize insufficient coverage and selectivity. Motion validity
is finite for all samples, with $1.03\%$ joint-range violations, but FID/Div,
music semantics, structure, style, emotion, and human ratings are not yet available.
We therefore report no complete music-adaptation or dance-quality result for M3.
