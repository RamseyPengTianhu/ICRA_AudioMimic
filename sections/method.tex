...
expand the conditioning, reconstruction, and continuation mechanisms.
...
$d_i=i/15$ to obtain $\hat c_i$ (Fig.~\ref{fig:music_detail}). A coverage mask
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/foredance/music-conditioning-detail.pdf}
\caption{\textbf{Physical-time music conditioning.} Forecast features are
projected with their lead times and interpolated at motion-step times. The
aligned features drive separate residual FiLM modules in the structure and
residual branches. Coverage masking bypasses modulation outside valid support.}
\label{fig:music_detail}
\end{figure*}
...
(Fig.~\ref{fig:dc}). Codebook and commitment losses train the quantized
...
\caption{\textbf{Learning complementary structure and detail.} A discrete
anchor and continuous offset form the full code. Full and structure-only
reconstructions share one boundary-conditioned decoder. Pose colors illustrate
complete versus selected supervision; pale limbs remain in the reconstruction
but are outside the selected native view. Geometry is schematic; flames mark
learned modules.}
\label{fig:dc}
\end{figure*}
...
\label{sec:cof}
Teacher Forcing supplies recorded history and a recorded starting state;
streaming inference supplies both from the model's earlier output. Commit
Forcing changes this conditioning transition while retaining recorded
supervision for the next segment (Fig.~\ref{fig:cof}).
...
stop-gradient. Their common committed segment updates the two parts of the
next context:
\begin{equation}
...
The next recorded structural target $q^*$ and full latent
...
\label{eq:rebase}
\end{equation}
Thus the structure and residual losses retain a common recorded target
...
\caption{\textbf{Paired context and residual rebasing.} TF takes recent history
and boundary state from a recorded segment; CoF derives both from the same
generated commitment. At right, residual rebasing moves the residual's origin
to the sampled structure while retaining the recorded target latent. Tile
counts and geometric positions are schematic.}
\label{fig:cof}
