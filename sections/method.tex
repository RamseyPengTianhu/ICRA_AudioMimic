...
anchor and continuous offset form the full code. Both paths share one
state-conditioned decoder and produce complete motions. Filled cells select
coordinates for supervision in four blocks: native motion, native velocity, Cartesian position,
and Cartesian velocity. Counts are schematic; flames mark learned modules.}
\label{fig:dc}
\end{figure*}
...
TF uses recorded history and boundary state. CoF first samples a plan from a
recorded seed context, appends its committed prefix to history, and derives the
...
recorded target latent while changing the structural origin. Tile counts and
