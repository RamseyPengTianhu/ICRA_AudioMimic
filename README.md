...
Figure 1 uses `figures/foredance/overview_framework_h8_v9.pdf`, exported from the author-approved editable `overview_framework_h8_v9.svg`; matching full-size and panel previews are stored alongside it. The original `overview.svg` and `overview.pdf` are preserved. Panel b shows decoded motion-prefix commitment, with original latent prefixes updating History and committed reference motion updating Boundary State. Panel c explicitly separates SMPL human-motion retargeting from D+C training on G1 robot-native motion; neutral gray connectors indicate training-data use. G1 poses are model-rendered illustrations, not hardware photographs or verified paired human/robot retargeting frames. The figure retains the H8 configuration. The reading copy `ForeDance.pdf` matches `build/main.pdf`. Earlier figure-building scripts describe the original layout and should not overwrite this approved version.
